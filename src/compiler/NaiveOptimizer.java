...
//    static File inputFile = new File("examples/mod of PowerOfTwo.hex");
    ...
    public static void main(String[] args) throws IOException {
        ...
        NaiveTypechecker.Body moduleBody = (NaiveTypechecker.Body)polisher.semanticAnalysis(b, polisher.builtins);
		...
		if (b instanceof Body body) {
			...
			if (co.s.equals("%") && co.operands.size() == 2
					...
					&& refinementOperationEqual(rt, context,
							new CoreOp("==",
									new CoreOp("&", new AliasParse("x"),
											new CoreOp("-", new AliasParse("x"), new NumberParse("1"))),
									new NumberParse("0")))) {

				CoreOp subst = new CoreOp("&", co.operands.get(0),
						new CoreOp("-", co.operands.get(1), new NumberParse("1")));
				co.s = subst.s;
				co.operands.clear();
...
//                System.out.println("OPTIMIZE");
	...
	boolean refinementOperationEqual(RefinementType rt, Context context, CoreOp co) {
		if (rt.inheritType instanceof StructType st && st.vars.size() == 1) {
	...
	boolean operationEqual(Block b1, Block b2, String var) {
		if (b1 instanceof Body b && b.expr.size() == 1 && b.expr.get(0)instanceof CoreOp op && op.s.equals("return")
				&& op.operands.size() == 1) {
...
//            System.out.println("OPSSSS");
			if (!o1.s.equals(o2.s) || o1.operands.size() != o2.operands.size())
				return false;
